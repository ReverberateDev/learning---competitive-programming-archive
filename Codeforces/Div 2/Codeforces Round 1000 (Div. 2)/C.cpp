...
void solve(){
    ...
    int arr[n + 1];
    for(int i = 1; i <= n; i++){
        edges[i] = {adjlist[i].size(), i};
        arr[i] = edges[i].first;
    }
    sort(edges.begin(), edges.end());
    reverse(edges.begin(), edges.end());
    int best_edges = edges[0].first;

    int ans = -1e18;
    root = new node(1, n, arr);

    for(int i = 0; i < n; i++){
        if(edges[i].first < best_edges){
            break;
        }
        auto [no_edges, idx] = edges[i];
        //first vertex found, modify
        root->set(idx, idx, 0);
        ...
        int new_edges = root->range_max(1, n);
        if(DEBUG){
            cout << "1st and 2nd edges\n";
            cout << no_edges << " " << new_edges << '\n';
        }

        //set ans
        ans = max(ans, no_edges + new_edges - 1);

        //reset**
        root->set(idx, idx, no_edges);
        for(auto i : adjlist[idx]){
            root->add(i, i, 1);
        }
    }
